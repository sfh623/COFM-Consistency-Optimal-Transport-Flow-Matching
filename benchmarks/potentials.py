...
class BasePotential(nn.Module):
    ...
    def push(self, input_, create_graph=True, retain_graph=True):
        ...
        output = autograd.grad(
            ...
            only_inputs=True,
            grad_outputs=torch.ones_like(input_[:, [0]])
        )[0]
        return output
...
class ScaledPotential(BasePotential):
    ...
    def forward(self, input_):
        return self.scale * self.potential(input_)
...
class ShiftedPotential(BasePotential):
    ...
    def forward(self, input_):
        if ~self.todevice:
            self.todevice = True
            self.shift =  self.shift.to(input_.device)
        return self.potential(input_) + (self.shift * input_).sum(dim=1, keepdims=True)
...
class WeightedSumPotential(BasePotential):
    ...
    def forward(self, input_):
        ...
        for weight, potential in zip(self.weights, self.potentials):
            output += potential(input_) * weight
        return output
