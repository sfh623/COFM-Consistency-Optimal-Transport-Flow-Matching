...
from .benchmarktools import freeze, load_resnet_G
...
class PotentialMapBenchmark(Wasserstein2MapBenchmark):
    ...
    def _estimate_cost(self, estimate_size):
        ...
        X = self.input_sampler.sample(self.batch_size).to(self.device)
...
class Mix3ToMix10Benchmark(PotentialMapBenchmark):
    def __init__(self, dim=2, batch_size=1024, device='cuda'):
        ...
        input_sampler = distributions.RandomGaussianMixSampler(dim=dim, num=3,device = device)
        ...
        D1.load_state_dict(
            torch.load(
               f'/home/sfh/ICNNFlow-new/benchmarks/Mix3toMix10/{dim}_v1.pt',
                map_location=lambda storage, loc: storage
            )
        )
        ...
        D2.load_state_dict(
            torch.load(
                f'/home/sfh/ICNNFlow-new/benchmarks/Mix3toMix10/{dim}_v2.pt',
                map_location=lambda storage, loc: storage
            )
        ) 
        freeze(D2)
        ...
        class DeviceSampler:
            ...
            def sample(self, size):
                return self.sampler.sample(size).to(self.device)

        device_sampler = DeviceSampler(input_sampler, device)
        potential = potentials.standardize_potential(potential, device_sampler)
