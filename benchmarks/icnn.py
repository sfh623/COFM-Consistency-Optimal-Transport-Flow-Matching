...
class ConvexQuadratic(nn.Module):
    ...
    def forward(self, input_):
        ...
        quad = ((input_.matmul(self.quadratic_decomposed.transpose(1,0)).transpose(1, 0)) ** 2).sum(dim=1)
        linear = F.linear(input_, self.weight, self.bias)
        return quad + linear
...
class View(nn.Module):
    def __init__(self, *shape):
        ...
        self.shape = shape
    def forward(self, input_):
        return input_.view(-1, *self.shape)
...
class DenseICNN(nn.Module):
    ...
    def forward(self, input_):
        ...
        output = self.quadratic_layers[0](input_)
        for quadratic_layer, convex_layer in zip(self.quadratic_layers[1:], self.convex_layers):
            output = convex_layer(output) + quadratic_layer(input_)
        ...
        return self.final_layer(output) + .5 * self.strong_convexity * (input_ ** 2).sum(dim=1).reshape(-1, 1)
    
    def push(self, input_, create_graph=True, retain_graph=True):
        '''
        Pushes input by using the gradient of the network. By default preserves the computational graph.
        Apply to small batches.
        '''
        assert len(input_) <= self.batch_size
        output = autograd.grad(
            outputs=self.forward(input_), inputs=input_,
            create_graph=create_graph, retain_graph=retain_graph,
            only_inputs=True,
            grad_outputs=torch.ones_like(input_[:, :1], requires_grad=False)
        )[0]
        return output
    
    def push_nograd(self, input_):
        '''
        ...
        '''
        output = torch.zeros_like(input_, requires_grad=False)
        for i in range(0, len(input_), self.batch_size):
            input_batch = input_[i:i+self.batch_size]
            output.data[i:i+self.batch_size] = self.push(
                input_[i:i+self.batch_size],
...
class ConvICNN64(nn.Module):
    ...
    def forward(self, input_):
        reshaped_input = input_.reshape(len(input_), 3, 64, 64)
        ...
        return output + .5 * self.strong_convexity * (input_ ** 2).flatten(start_dim=1).sum(dim=1).reshape(-1, 1)
...
class FakeResNet64_D(ResNet_D):
    ...
    def forward(self, input_):
        return .5 * (input_ ** 2).flatten(start_dim=1).sum(dim=1).reshape(-1, 1) - \
        super(FakeResNet64_D, self).forward(input_.reshape(-1, 3, 64, 64))
...
class FakeUNet64_H(UNet):
    ...
    def push(self, input_):
        return super(FakeUNet64_H, self).forward(input_.reshape(-1, 3, 64, 64)).flatten(start_dim=1)
    ...
    def push_nograd(self, input_):
        ...
        with torch.no_grad():
            for i in range(0, len(input_), self.batch_size):
                input_batch = input_[i:i+self.batch_size]
                output.data[i:i+self.batch_size] = self.push(input_[i:i+self.batch_size]).data
        return output  