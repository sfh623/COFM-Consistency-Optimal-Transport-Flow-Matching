...
def compute_loss_hj(model, x_1, x_0, t, lambda_hj=0.5):
...
def compute_loss_hj2(model, x_1, x_0, t, lambda_hj=0.5):
    t = t.clone().detach().requires_grad_(True)
    x_t = (t * x_1 + (1 - t) * x_0)
    x_t = x_t.clone().detach().requires_grad_(True)
...
def compute_loss_back(model, x_1, x_0, t, t1=0, lambda_hj=0.5):
...
def compute_loss(model, x_1, x_0, t, t1=0, lambda_hj=1.0):
    ...
    #grad_psi_xval1,residue_HJ = torch.autograd.functional.jvp(gradx_psi, (x_t,t), v = (x_1-x_0 ,torch.ones_like(t)),create_graph=True) # 13.7
...
def train_flow_mnist(
    ...
    device = next(model.parameters()).device
    t0 = 0.0
    xtest = prior_sampler.sample(4096).to(device)
